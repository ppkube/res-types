...
type SeldonDeploymentLister interface {
	...
	List(selector labels.Selector) (ret []*v1.SeldonDeployment, err error)
...
// List lists all SeldonDeployments in the indexer.
func (s *seldonDeploymentLister) List(selector labels.Selector) (ret []*v1.SeldonDeployment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.SeldonDeployment))
	})
	return ret, err
}
...
type SeldonDeploymentNamespaceLister interface {
	...
	Get(name string) (*v1.SeldonDeployment, error)
...
// List lists all SeldonDeployments in the indexer for a given namespace.
func (s seldonDeploymentNamespaceLister) List(selector labels.Selector) (ret []*v1.SeldonDeployment, err error) {
...
// Get retrieves the SeldonDeployment from the indexer for a given namespace and name.
func (s seldonDeploymentNamespaceLister) Get(name string) (*v1.SeldonDeployment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("seldondeployment"), name)
	}
	return obj.(*v1.SeldonDeployment), nil
}
